...
@PropertySource： 读取properties属性配置文件可以实现外部化配置，在程序代码之外提供数据。
步骤：
...
2. 明确的在启动类上，加入@EnableTransactionManagement
...
@RestController：放在类上，创建控制器对象，注入到容器中。作用：复合了@Controller和@ResponseBody，使用这个注解，控制器方法返回值都是数据，没有视图。
...
@Qualifier：给引用类型赋值，使用byName。
_注：@Autowired，@Qualifier都是Spring框架提供的_
...
@Transactional：处理事务，放在service实现类的public方法上，表示此方法有事务。
