...
$
y = \left(1 + \frac{M}{2^{23}}\right) \cdot 2^{E-127}
$
...
$
\log_2(a^m) = m \cdot \log_2 a
$

我们要求的是平方根的倒数，也就是求 a的对数乘以-1/2 ：

$
\log_2\left(a^{-\frac{1}{2}}\right) = -\frac{1}{2} \cdot \log_2 a
$
...
$$
\begin{aligned}
&\quad \log_2 \left[ \left(1 + \frac{M}{2^{23}}\right) \cdot 2^{E-127} \right] \\\\
&= \log_2 \left(1 + \frac{M}{2^{23}}\right) + \log_2 2^{E-127} \\\\
...
$$
\log_2 y = \frac{1}{2^{23}} \cdot Y - 127
$$

那么，假设 a 为 y 的平方根倒数：

$$
\log_2 a = \log_2 y^{-\frac{1}{2}} = -\frac{1}{2} \log_2 y
$$

将 a 和 y 换成二进制形式：

$
\frac{1}{2^{23}} \cdot A - 127 = -\frac{1}{2} \left(\frac{1}{2^{23}} \cdot Y - 127 \right)
$
...
![img.png](https://cooooing.github.io/images/学习笔记/平方根倒数快速算法/手写过程.jpg)
