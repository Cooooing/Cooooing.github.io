...
1. requirepass：配置redis的访问密码。默认不配置密码。此参数不依赖 `protected-mode` 才生效；`protected-mode` 是 Redis 暴露在不安全网络环境下的保护机制。
...
4. no-appendfsync-on-rewrite：重写时是否可以不执行 appendfsync，默认no，可以保证数据安全性。
...
事务：把一组数据库操作放在一起执行。关系型数据库事务通常强调原子性，要么同时成功，要么同时失败。
Redis的事务：允许把一组redis命令放在一起执行，把命令序列化，然后一起执行。Redis 事务保证命令按顺序执行，但运行期命令错误不会回滚已执行命令。
...
13. 原主机恢复
* 在6379上执行：slaveof 127.0.0.1 6380
  * 让6379变为6380的从机
  * 或者在6379上执行：slaveof 127.0.0.1 6381
  * 让6379成为6381的从机，此时6381既是6380的从机，又是6379的主机。Redis 从库默认只读，可以读取但不能写入。
