...
1. 同一个输入在同一个散列函数下会得到确定的输出；如果两个散列值不同，那么原始输入一定不同。
2. 散列函数的输入和输出不是唯一对应关系的，如果两个散列值相同，两个输入值可能相同，也可能不同，这种情况称为“**散列碰撞**（collision，哈希碰撞）”。
3. 单向性是密码学哈希函数的性质，不是所有普通散列函数都具备的性质。
