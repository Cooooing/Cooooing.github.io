...
按大小合并，即**总是将元素少的树连接至元素多的树上**。
如果真正按秩合并，秩通常表示树的高度或近似高度；本文后面的实现记录的是集合大小，因此应称为按大小合并。
...
下面的并行示例用于说明拆分思路。实际工程中不能让多个线程直接并发修改同一个基于 `HashMap` 的并查集结构；需要分片构建后再合并，或者对共享结构加锁，否则会产生数据竞争。
