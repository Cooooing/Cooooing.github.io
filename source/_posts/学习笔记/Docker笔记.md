...
Docker Engine 的安装方式与发行版版本有关，旧版 CentOS 6 已不适合作为 Docker Engine 的常规安装环境。
CentOS 7 可以按 Docker 官方仓库方式安装；旧教程中也常见 `yum install docker -y` 的发行版包安装方式。
...
查看某个命令帮助信息：`docker command --help`
...
docker是客户端-服务器（C/S）架构，通过远程API来管理和创建容器。
docker通过镜像来创建容器。
