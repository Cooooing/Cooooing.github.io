...
又又又被攻击了，这次是通过 frp 挂在公网的 rdp。方便我远程访问家里云。

起初是看到了v站的一篇帖子：[求助，被入侵了，如何减小损失？](https://v2ex.com/t/1092714)
大致内容就是他通过 frp 挂在公网的 rdp 被入侵了，常用的网站密码都泄露了。
...
另外就是加 ip 黑名单，这个不现实。因为 frp 服务转发后，所有请求都是从公网的服务器过来的。

但是在服务器的层面加ip黑名单是可以的。
bash脚本我就不是很熟了，连夜和gpt一起写了个脚本，检测 frp 的日志，把频繁访问的都加到防火墙的ip黑名单里。
最后配置个定时任务运行脚本。就完成了。
...
CHAIN_NAME="FRP-BLACKLIST"                  # 自定义防火墙链名称
...
另外，直接用 `iptables` 添加的规则重启后可能丢失，需要配合 `iptables-save`、`iptables-persistent` 或发行版对应的防火墙持久化方案。
