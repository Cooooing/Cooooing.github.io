...
![Levenshtein距离的数学定义.png](https://cooooing.github.io/images/编程记录/计算字符串相似度/Levenshtein距离的数学定义.png)
...
下面的实现会写入全局临时表，并且为了在 SQL 中调用函数使用了自治事务，所以不应该标记为 `DETERMINISTIC`。
...
CREATE OR REPLACE FUNCTION func_get_similarity_ratio(str VARCHAR2, target VARCHAR2) RETURN NUMBER IS
    -- 使用自治事务
...
~~~oraclesqlplus
create FUNCTION func_get_similarity_ratio(str VARCHAR2, target VARCHAR2) RETURN NUMBER IS
    PRAGMA AUTONOMOUS_TRANSACTION;
    n     NUMBER := LENGTH(str);
    m     NUMBER := LENGTH(target);
    i     NUMBER;
