...
虽然我也会写 vue，用些组件也能做些好看的页面。但始终差点意思。
...
比如写这篇文章时使用的 v2.5.1 只支持单窗口应用、没有托盘图标、缺少一些系统事件（全局热键、聚焦失焦等）。
...
wails 虽然有缺点，但是也只能用了。毕竟 GUI 框架没什么可以选。
...
`ssh.InsecureIgnoreHostKey()` 会跳过服务端主机密钥校验，只适合临时测试。实际使用时应该保存并校验服务端 host key，避免中间人攻击。
...
得去深入学习下 io 流 和 多线程
桌面应用就这样吧，主要当时的 wails v2 功能还是太少了。等 v3 正式版再来用 go 玩玩桌面应用。
