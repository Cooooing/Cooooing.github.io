...
- Kafka 采用 `sendfile()`（零拷贝）：
    - **步骤 1**：内核将 **磁盘 Page Cache** 中的数据发送到 **Socket Buffer**，绕过用户态，不经过用户缓冲区。
...
Kafka 服务器支持故障转移的方式就是使用会话机制。每台 Kafka 服务器启动后会以会话的形式把自己注册到ZooKeeper 服务器上。一旦该服务器运转出现问题，与ZooKeeper 的会话便不能维持从而超时失效，此时 controller 会感知 broker 下线，并为受影响分区重新选举 leader，使可用副本继续提供服务。
...
Kafka 消息格式共经历过3次变迁，它们被分别称为 V0、V1和 V2版本。Kafka 0.11 起引入 V2 record batch，现代版本通常以 V2 记录批为主。

![V1版本的消息格式.png](https://cooooing.github.io/images/读书笔记/《apache%20Kafka实战》读书笔记-基本概念/V1版本的消息格式.png)
...
![topic和partition.png](https://cooooing.github.io/images/读书笔记/《apache%20Kafka实战》读书笔记-基本概念/topic和partition.png)
...
![消息位移与消费者位移.png](https://cooooing.github.io/images/读书笔记/《apache%20Kafka实战》读书笔记-基本概念/消息位移与消费者位移.png)
...
| 3.0.x  | 继续推进KRaft模式    | 继续支持ZooKeeper模式；KRaft仍在演进中，后续 3.3 起标记为生产可用，4.0 移除 ZooKeeper 模式 |
