...
如果组合索引的前导列取值不多，且查询条件没有包含前导列，Oracle 仍可能用到该组合索引，这种扫描被称为跳跃索引，不过应用场景不多。
跳跃索引（Index Skip Scan）是Oracle数据库中的一种特殊的索引访问方式。它主要用于组合索引缺少前导列条件、且前导列基数较低的场景，不是专门用于处理 NULL 的访问方式。
跳跃索引的工作原理是按前导列的不同取值把组合索引视作多个逻辑子索引，再分别扫描这些子索引，从而减少不必要的全索引扫描或全表扫描。
