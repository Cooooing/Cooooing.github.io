...
2. 首次执行该SQL指令结束后，SGA内存区的共享池里已经保存了该SQL唯一指令HASH值，并保留了语法语意检查及执行计划等相关解析动作的劳动成果，当再次执行该SQL时，由于该SQL指令的HASH值和共享池里保存的相匹配了，所以之前的硬解析动作就无须再做，不仅跳过了相关语法语意检查，对于该选取哪种执行计划也无须考虑，直接拿来主义就好了。
...
下面介绍CKPT，**检查点会协调脏数据块写出，并更新检查点相关信息**。
如果更新操作的一直不提交，数据缓存区的数据会被写入磁盘中吗？
答案是会的，**因为DBWR将数据缓存区数据写入磁盘，不是由Commit决定的，而是由检查点、脏块压力和缓存空间等因素共同影响的。**
**DBWR负责把数据缓存区中的脏数据块写入数据文件；LGWR负责把重做日志缓存写入重做日志文件。数据库会遵守先写日志的约束，确保脏块写出前相关 redo 已经落盘。**
**因为变更需要有日志记录，否则会发生数据丢失的问题。**
...
5. **CKPT (Checkpoint Process)**:
   用于检查点协调和检查点信息更新。检查点越频繁，DBWR需要推进的写出进度越积极，异常恢复通常越快，但也可能增加写出压力。
...
3. 在数据缓存区中创建object_id=29的前镜像，前镜像数据也会写进磁盘的数据文件里（回滚表空间的数据文件)，从缓存区写进磁盘的规律前面已经说过了，由DBWR写入，并受检查点和缓存压力等因素影响，当然也别忘记了这些动作都会记录日志，并将其写进日志缓存区，劳模LGWR还在忙着将日志缓存区的数据写入磁盘形成redo文件呢。
...
记录前镜像为什么也需要记录日志？
前镜像会记录在SGA的数据缓存区（Undo缓存区）里，后续由DBWR写入磁盘，相关 redo 由 LGWR 写入重做日志。
...
切记 commit 可不是写数据块的动作。commit 主要等待 LGWR 写 redo 成功；将 DATA BUFFER 中的脏数据块写入数据文件是 DBWR 的职责，检查点会参与协调和推进写出进度。
